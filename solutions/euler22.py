...
"""
from utils.strings import alphabetical_value

names = sorted(open('../resources/names.txt', 'r').read().replace('"', '').split(','))
s = sum([(pos + 1) * alphabetical_value(names[pos]) for pos in range(len(names))])
print(s)
